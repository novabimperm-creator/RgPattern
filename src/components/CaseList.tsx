...
function EmptyState({
  ...
  const title = hasCases
    ? "Ничего не найдено"
    : ephemeral
      ? "Пропали случаи и логины"
      : "Пока нет случаев";
  const description = hasCases
    ? "Попробуйте другой запрос — поиск идёт по системе, заключению, описанию и заметкам."
    : ephemeral
      ? "Railway заново собрал контейнер. Без Volume удаляются не только снимки, но и суперадмин с пользователями — файл учёта лежит рядом с архивом. Подключите диск /app/data, дождитесь редеплоя, создайте суперадмина заново и загрузите случаи. GitHub эти данные не хранит."
      : canEdit
